...
def optimizer(loss, var_list):
    ...
    try:
        ...
        print([v for v in var_list if v.name == "D/d0/w:0"][0])
        print([v for v in var_list if v.name == "D/g0/w:0"][0])
    except:
        print("None")
    return optimizer
...
#                session.run(v.assign(self.weightsD[i]))
            ...
            for step in xrange(self.num_steps):
                ...
                #gg = session.run([self.G], {self.z: np.reshape(z, (self.batch_size, 1))})
                #print(gg)

                if step % self.log_every == 0:
                    print('{}: {}\t{}'.format(step, loss_d, loss_g))
                    print([v.eval() for v in tf.all_variables() if v.name == "D/d0/w:0"])
                    print([v.eval() for v in tf.all_variables() if v.name == "G/g0/w:0"])
    ...
    def _samples(self, session, num_points=10000, num_bins=100):
        ...
        plt.plot(np.linspace(-self.gen.range, self.gen.range, len(pd)), pd)
        plt.show()
...
def parse_args():
    ...
    parser.add_argument('--anim', type=str, default=False,
